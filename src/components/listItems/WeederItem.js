...
const WeederItem = ({ item }) => {
  const { currentReport } = useSelector((store) => store.reports);
  const [trees, setTrees] = useState(0);
  const [acres, setAcres] = useState(0);

  const queryWeedingData = () => {
    db.transaction((tx) => {
      tx.executeSql(
        `SELECT * FROM weedings WHERE report_id = ? AND worker_matricule = ?`,
  ...
  const saveWeeding = () => {
    db.transaction((tx) => {
      tx.executeSql(
        ...
        () => console.log("Weeding succefully saved")
      );
    });
  };
